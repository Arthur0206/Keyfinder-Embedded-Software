...
/**
 * Callback when the device has read its own BD ADDR.
 */
typedef void (*gapRolesBDAddrRead_t)( uint8 * bdAddr);
...
typedef struct
{
  ...
  gapRolesBDAddrRead_t     pfnBDAddrRead;   //!< When the bd addr is read
} gapRolesCBs_t;
